...
def handle_space_bar(event):
    global waiting_for_space, event_counter
    if collecting_data and waiting_for_space:
        with data_lock:
            event_counter += 1
        update_event_label()
        print(f"Evento {event_counter} activado")
        if event_counter == 9:  # Detener después del último evento
            stop_sampling()
            root.quit()
...
def collect_aura_data():
    ...
    with open(raw_file, mode='w', newline='') as raw_csv, open(fft_file, mode='w', newline='') as fft_csv:
        ...
        raw_writer.writerow(['Event', 'Time', 'F3', 'Fz', 'F4', 'C3', 'C4', 'P3', 'Pz', 'P4'])
        fft_writer.writerow(['Event', 'Time'] + [f"{band}_{channel}" for band in ['Delta', 'Theta', 'Alpha', 'Beta', 'Gamma']
                                                for channel in ['F3', 'Fz', 'F4', 'C3', 'C4', 'P3', 'Pz', 'P4']])
        while collecting_data:
            ...
            try:
                ...
                if raw_sample:
                    with data_lock:
                        raw_writer.writerow([event_counter, raw_timestamp] + raw_sample[:8])
                    print(f"Datos RAW: Evento: {event_counter}, {raw_sample[:8]}")
                fft_sample, fft_timestamp = aura_inlet2.pull_sample(timeout=0.1)
                if fft_sample:
                    with data_lock:
                        fft_writer.writerow([event_counter, fft_timestamp] + fft_sample[:40])
                    print(f"Datos FFT: Evento: {event_counter}, {fft_sample[:40]}")
            except Exception as e:
        ...
        try:
            ...
            if fft_sample:
                ...
                if baseline is not None:
                    ...
                    if ce < lower_bound or ce > upper_bound:
